...
1. **数据准备**
    ...
    - 将郑州树种属性数据集（郑州树种数据集.xlsx）放入`./data`目录

    - 确保`./output`目录存在（用于保存模型、结果文件）
...
├── output/               # 结果输出目录
│   ├── label_encoder.pkl # 树种标签编码器
│   ├── attr_scaler.pkl   # 属性特征标准化器
│   ├── best_model.h5     # 训练完成的最佳模型
│   ├── training_history.json # 模型训练历史数据
...
- 模型在测试集上的准确率可达约 70%（最终精度取决于数据质量和样本量）
...
- 郑州树种属性数据集为用户提供的整理后数据，包含树种的文献验证、本地属性等信息
