# DL-test
# 面向城市树种分类的多源数据驱动轻量化全局 - 局部特征协同网络

## 项目介绍

本项目结合多源遥感影像（Sentinel-2 光学影像、Sentinel-1 雷达影像、高分二号高分辨率影像）与全球城市树种数据集(GUTS)，采用Swin Transformer+MLP多模态融合深度学习模型，实现郑州城市区域的树种精准分类，解决单一属性数据分类的局限性，提升分类的客观性和泛化能力。
...
1. **双分支特征提取**
    ...
    - 遥感影像分支：使用 Swin Transformer 作为骨干网络，提取影像的空间长程依赖特征和光谱特征，加入ASPP多尺度特征增强模块

    - 属性数据分支：使用多层感知机（MLP）提取属性数据的特征
...
3. **损失函数优化**
    ...
    - 使用SparseCategoricalFocalCrossentropy损失函数，解决树种类别不平衡问题，提升小样本树种的识别精度

## 结果说明

- 模型在测试集上的准确率较高（最终精度取决于数据质量和样本量）

- 可输出郑州城市区域的树种分布专题图、分类报告、混淆矩阵、Kappa系数等评估结果
